...
bool Guild::Create(Player* pLeader, std::string_view name)
{
    ...
    // Cluster mode: the guild service owns guild creation. Real players reach
    // it through the gateway petition flow; in-process bot sessions call the
    // service directly (fork-only path, same pattern as TC9GuildAcceptInvite).
    // GM ".guild create" stays in-process, picked up by the service refresh.
    if (sToCloud9Sidecar->ClusterModeEnabled() && pLeaderSession->IsBot())
...
// Cluster mode, bot-only path: the guild service allocates the guild id and
// inserts the guild, default ranks and leader rows in one transaction (and
// hydrates its cache + publishes guild.created). Mirror that exact state in
// memory without touching the database again. Other shards mirror it from
// the guild.created event (the consumer skips this shard, the guild is
// already registered).
...
bool Guild::MirrorClusterCreated(uint32 guildId, std::string_view name, ObjectGuid leaderGuid, std::vector<ObjectGuid> const& memberGuids)
{
    ...
    for (ObjectGuid memberGuid : memberGuids)
        _MirrorAddMember(memberGuid, GR_INITIATE);
